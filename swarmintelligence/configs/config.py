...
tests.py → Quick testing or smoke-testing script.
...
- **testing_utils.py**  
  Generate dummy DataFrames/models/images for quick unit tests, checkpoint/retrieve tests artifacts.
...
The `eigenlib/tests.py` module uses `unittest` to exercise nearly every helper and class:
...
* All codes must be functional just by launching the code inside python, always include the code and the launch method already configured to tests it.
...
class ExampleClass():
    ...
    def method_2(self, argument_1, argument_2='tests'):
        <we avoid comments.>
        my_class = MyClass()
